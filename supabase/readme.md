...
- Claves actuales se declaran en `src/pages/client.js` (y copia en `supabase/client copy.js`).
- Recomendado: mover a variables de entorno Vite (`VITE_SUPABASE_URL`, `VITE_SUPABASE_KEY`) y leerlas en el cliente.
...
- Reemplaza `MAPBOX_TOKEN` en `src/pages/dashboard.jsx` por tu token de Mapbox.
...
- Para evitar exponer claves, usa variables de entorno en producción.
- La PokéAPI tiene rate limits; maneja reintentos si consumes mucho tráfico.
- El mapa requiere conexión; si falla, se muestra mensaje en dashboard.
